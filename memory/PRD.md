...
## «Создать правило из операции» (13.05.2026)
- [x] **Frontend** (`TransactionsPage.jsx`): новый пункт меню `... → «Создать правило из операции»` (Bot-иконка) в каждой строке таблицы.
- [x] Диалог `create-rule-dialog`:
   - Превью карточкой «Из операции: <описание>»
   - Поле «Паттерн» с авто-предложением: выбирается самое длинное слово ≥4 символов из описания, fallback на первые 20 символов
   - Live-подсказка: «Срабатывает, если описание содержит подстроку «X»»
   - Селекты Статья / Направление (с опцией «Не менять» = null)
   - Чекбокс «Применить к существующим операциям» (по умолчанию включён) — после создания правила найдёт все операции с совпадающим описанием и заполнит пустые Статья/Направление через `bulk-apply-rules`
- [x] Сохранение: `POST /api/auto-rules` + опциональный `POST /transactions/bulk-apply-rules` с `search=pattern`. Тост: «Правило создано · обновлено N существующих операций» либо просто «Правило создано».
- [x] **Тестирование (13.05.2026)**: создал операцию «BIEDRONKA market przelew», открыл меню → диалог открылся с авто-паттерном «BIEDRONKA», селекты и чекбокс работают. UI screenshot подтвердил.

## «Умные предложения» на странице Авто-правил (13.05.2026)
- [x] **Backend** `GET /api/auto-rules/suggestions?limit=N` (`routes/projects.py`): анализирует все транзакции пользователя без `category_id` (только income/expense). Из описаний извлекает токены ≥4 символов (через regex `[^\w]+`, UNICODE-safe — работает с кириллицей и латиницей). Фильтрует stopwords (`tytuł`, `przelew`, направления и пр.), исключает токены, уже покрытые активными правилами. Группирует по токену, сортирует по `(count desc, total_amount desc)`. Возвращает топ-N с полями `pattern, count, total_amount, samples[3]`. Дополнительно — `total_uncategorized`.
- [x] **Frontend** (`AutoRulesPage.jsx`): новая фиолетовая карточка «Умные предложения» (Sparkles-icon) над таблицей правил. Каждое предложение — отдельная плашка с: бейджем паттерна, счётчиком, суммой, 2 примерами, селектами Статья/Направление и кнопкой «Создать правило». При клике: создаётся правило + сразу применяется к существующим операциям через `bulk-apply-rules`. Тост: «Правило … создано · обновлено N операций». Состояние перезагружается.
- [x] **Тестирование (13.05.2026)**: seed 3 ALLEGRO + 2 BIEDRONKA + 1 unique → backend выдал ожидаемые кандидаты + реальные данные пользователя (антон 9, продажа 4, материалы 4). UI screenshot подтвердил рендер карточки с 3 предложениями и контролами.

## Чистый капитал + правильный учёт операций с заёмных счетов (13.05.2026)
- [x] **Backend fix**: `_not_loan_op` (`analytics.py`) и main_match в `transactions.py` теперь исключают **только переводы** с участием loan-счетов (взятие/возврат займа). Прямые income/expense на loan-счёте возвращаются в Доходы/Расходы PLN/EUR как реальные движения денег бизнеса (например, оплата подрядчика с кредитной линии).
- [x] **Backend extension**: `/api/analytics/summary` теперь возвращает `assets_balance`, `liabilities_balance` (сумма current_balance loan-счетов, обычно отрицательная), `net_worth = assets + liabilities`. Конвертация EUR → PLN через текущий NBP.
- [x] **Frontend** (`DashboardPage.jsx`): новый виджет `net-worth-card` сразу после блока 4-х метрик — три колонки (Активы / Займы / Чистый капитал) с цветовой индикацией. При отрицательном NW — предупреждение «Долги сейчас больше, чем имеется на счетах…» со ссылкой на график динамики. Карточка скрывается, если у пользователя нет loan-счетов.
- [x] **Тестирование**: backend вернул правильные значения с тестовым loan-счётом (-15000 zł). UI screenshot: «Чистый капитал −9 552 zł» с alert-сообщением ✅.

## Разбивка займов по счетам (13.05.2026)
- [x] **Backend** `loans_summary` теперь возвращает `per_account: [{id, name, currency, current_balance, received_by_cur, received_count, repaid_by_cur, repaid_count}]`. Mongo `$group` теперь группирует одновременно по `account_id` и `currency`.
- [x] **Frontend** `LoansSummary` — под основными карточками появилась секция «Разбивка по счетам» с одной строкой на каждый loan-счёт: имя · поступления · погашения · остаток. Пустые столбцы помечены `—`. Это позволяет сразу видеть, какой именно счёт стоит за общей цифрой «Получено».
- [x] **Тестирование (13.05.2026)**: создал 2 loan-счёта (Тест-Аликор EUR, Тест-Минск PLN), по 1 transfer каждый → per_account отобразил оба отдельной строкой с правильными суммами и валютами. UI screenshot подтвердил.

## Drill-down: клик по строке Займа фильтрует операции (13.05.2026)
- [x] **Frontend** (`TransactionsPage.jsx`): строки в «Разбивка по счетам» теперь кликабельны (только при наличии операций — hover-ring и cursor-pointer). По клику: `setFilters({ type: 'transfer', selectedAccountIds: [accId] })` + плавный скролл наверх. В таблице остаются ровно те операции, которые составили сумму «Получено/Погашено» для этого счёта.
- [x] **Тестирование**: создал 2 loan-счёта по 1 transfer на каждый → клик по «DRILL-Минск» отфильтровал ровно 1 операцию. Скриншот подтвердил.

### v2.5.5 (13.05.2026) — Смена валюты счёта
- [x] **Backend** (`routes/accounts.py`): `PUT /api/accounts/{id}` детектирует изменение `currency` и вызывает `_recalc_account_amount_base` + `_recalc_balance` → пересчитывает `amount_base` для всех source-транзакций счёта и `to_amount_base` для всех incoming transfer-ов (учитывая ручной `to_amount`). Конвертация через текущий курс NBP.
- [x] **Frontend** (`SettingsPage.jsx`): в диалоге редактирования счёта селект «Валюта» уже редактируемый; добавлен `window.confirm` при смене валюты + inline-предупреждение `⚠ Транзакции будут пересчитаны по курсу NBP (FROM → TO)`.
- [x] **Тестирование** (curl): создал PLN-счёт с initial_balance=1000, добавил EUR 100 income (amount_base=425.24 PLN). PLN→EUR → amount_base=100 EUR, balance=1100 EUR ✓. EUR→PLN → amount_base=425.24 PLN, balance=1425.24 PLN ✓. Smoke-screenshot Settings page подтвердил работоспособность UI.
...
- [ ] Рефакторинг `TransactionsPage.jsx` (>1800 строк → вынести `LoansSummary`, `PeriodSummary`, `BulkActionBar`)
