...
- [x] **Проблема**: при переводе между счетами в разных валютах (PLN→EUR и т.п.) до сих пор использовался только автоматический NBP-курс; реальный курс банка отличался → расхождение балансов.
- [x] **Backend** (`/app/backend/routes/transactions.py`, `/app/backend/models.py`): в `TransactionCreate` добавлено опциональное поле `to_amount` (сумма в валюте получателя). В `create_transaction` и `update_transaction`: если `to_amount > 0` И валюты source/target различаются — `to_amount_base = to_amount`, `exchange_rate = round(amount / to_amount, 6)`. Иначе — fallback на `calc_amount_base` (NBP/manual rate).
- [x] **Frontend** (`TransactionsPage.jsx`): при типе «Перевод» и разных валютах источника/получателя отображается amber-блок с двумя связанными полями: «Сумма к получению (TO_CUR)» (`form-to-amount`) и «Курс (FROM_CUR/TO_CUR)» (`form-manual-rate`). Изменение одного авто-пересчитывает другое. При открытии редактирования cross-currency перевода поля префилл из `to_amount_base` и `amount / to_amount_base`. Если оба пустые — backend применит NBP/manual из настроек.
- [x] **Тестирование (13.05.2026)**: curl — `1000 PLN → 230 EUR` создан с `exchange_rate=4.347826`; редактирование без `to_amount` → авто NBP 235.42; редактирование c `to_amount=250` → `rate=4.0`. UI screenshot подтвердил префилл (230 / 4.347826), линковку полей (rate=4.0 → to_amount=250) и сохранение в БД (250 / 4.0).
